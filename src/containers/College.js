...
import Data from "../helpers/College.json";
import { useParams } from "react-router-dom";

const College = (props) => {
  const [data, setData] = useState(Data.college[0]);
  const { name } = useParams();
  useEffect(() => {
    console.log(name);
    const d = Data.college.filter((ele) => ele.url === name)[0];
    setData(d);
  }, [name]);
  return (
    <div className={styles.outerWrapper}>
      ...
      <p className={styles.founderContent}>
        ...
        and seen as a negative stereotype. With findhope{" "}
        {data.url.toLocaleUpperCase()} we want to change this attitude, We want
        to bring free, accessible and quality mental health care to our campus.
      </p>
